...
class FinetuneImage(Dataset):
    def __init__(self, state, arbitrary_mask_percent=0, dataset_ratio=(0.8, 0.1, 0.1), class_label_exist=False,
                 class_name=None, repeats=200, large_mask=False, **args
                 ):
        self.state = state
        self.args = args
        ...
        self.num_images = len(self.bbox_path_list)
        self._length = self.num_images

        if state == "train":
            self._length = self.num_images * repeats
        elif state == "validation":
            self._length = self.num_images * 4

    def __getitem__(self, index):
        bbox_path = self.bbox_path_list[index % self.num_images]
        file_name = os.path.splitext(os.path.basename(bbox_path))[0] + '.jpg'
        # dir_name = bbox_path.split('/')[-2]
        img_path = os.path.join('dataset/fine_tuning/fabric_sofa/raw', file_name)  # Path setting
        ...
        bbox_index = label_list.index(self.class_name)
        ...
        ref_image_tensor = get_tensor()(ref_image_tensor)
        # ref_image_tensor = get_tensor_clip()(ref_image_tensor)

        # Mask Augmentation
        if self.large_mask == True:
            if random.choice([True, False]):
                bbox[0] = int(bbox[0] * 0.75) if bbox[0] * 0.75 > 0 else 0
                bbox[1] = int(bbox[1] * 0.75) if bbox[1] * 0.75 > 0 else 0
                bbox[2] = int(bbox[2] * 1.5) if bbox[2] * 1.5 < img_p.size[0] else img_p.size[0]
                bbox[3] = int(bbox[3] * 1.5) if bbox[3] * 1.5 < img_p.size[1] else img_p.size[1]
