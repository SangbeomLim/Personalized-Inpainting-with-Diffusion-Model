...
class LSUNDataset(data.Dataset):
    def __init__(self, state, arbitrary_mask_percent=0, dataset_ratio=(0.8, 0.1, 0.1), class_label_exist=False,
        ...
        if state == "train":
            ...
            per_dir_file_list = per_dir_file_list[:280000]  # 90%
            ...
            per_dir_file_list = per_dir_file_list[280000:290000]  # 10%
            for file_name in per_dir_file_list:
                if file_name not in bad_list:
                    self.bbox_path_list.append(os.path.join(bbox_dir, file_name))
        else:
            ...
            end_index = int(len(per_dir_file_list) * self.dataset_ratio[1]) + start_index
            # per_dir_file_list = per_dir_file_list[start_index:end_index] # 10%
            per_dir_file_list = per_dir_file_list[290000:300000]
            for file_name in per_dir_file_list:
                if file_name not in bad_list:
                    self.bbox_path_list.append(os.path.join(bbox_dir, file_name))
    ...
    def __getitem__(self, index):
        ...
        img_path = os.path.join('dataset/lsun/raw', file_name)  # Path setting
        ...
        if self.large_mask == True:
            if random.choice([True, False]):
                bbox = [i * 1.5 for i in bbox if random.choice([True, False])]
        ...
        return {"GT": image_tensor_resize, "inpaint_image": inpaint_tensor_resize, "inpaint_mask": mask_tensor_resize,
                "ref_imgs": ref_image_tensor, "class_label": torch.tensor(class_label)}

    def __len__(self):
        return self.length

    def test_get_item(self, index, model):
        bbox_path = self.bbox_path_list[index]
        file_name = os.path.splitext(os.path.basename(bbox_path))[0] + '.jpg'
        # dir_name = bbox_path.split('/')[-2]
        ...
        return {"source_img": img_p_np, "reference_img": ref_image, "mask_img": mask_img, "class_label": class_label,
                "bbox": bbox_pad}
