...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
        ...
        self.catption_exist = catption_exist
    ...
    def get_input(self, batch, k):
        ...
        class_label = class_label.to(memory_format=torch.contiguous_format).float()
        if self.catption_exist:
            return x, inpaint, mask, reference, caption
        else:
            return x, inpaint, mask, reference, class_label
...
class InversionLatentDiffusion(DDPM):
    """main class"""
    ...
    def __init__(self,
        ...
        self.proj_out = nn.Linear(1792, 768)
    ...
    def get_learned_conditioning(self, c_text, c_image):
        if self.cond_stage_forward is None:
            if hasattr(self.cond_stage_model, 'encode') and callable(self.cond_stage_model.encode):
                c = self.cond_stage_model.encode(c_text, c_image)
                if isinstance(c, DiagonalGaussianDistribution):
                    c = c.mode()
            else:
                c = self.cond_stage_model(c_text, c_image)
        else:
            assert hasattr(self.cond_stage_model, self.cond_stage_forward)
            c = getattr(self.cond_stage_model, self.cond_stage_forward)(c_text, c_image)
        return c
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        x, inpaint, mask, reference, text = super().get_input(batch, k)
        if bs is not None:
            x = x[:bs]
            inpaint = inpaint[:bs]
            mask = mask[:bs]
        ...
        z_inpaint = self.get_first_stage_encoding(encoder_posterior_inpaint).detach()  # [20,4,64,64]
        mask_resize = Resize([z.shape[-1], z.shape[-1]])(mask)
        z_new = torch.cat((z, z_inpaint, mask_resize), dim=1)  # [20,9,64,64]
        ...
        if self.model.conditioning_key is not None:
            ...
            if not self.cond_stage_trainable or force_c_encode:
                if isinstance(xc_image, dict) or isinstance(xc_image, list):
                    # import pudb; pudb.set_trace()
                    c = self.get_learned_conditioning(xc_image)
                else:
                    c = self.get_learned_conditioning(xc_text, xc_image.to(self.device))
                    c = self.proj_out(c)
                    c = c.float()
            else:
                c = xc_image
    ...
    def shared_step(self, batch, **kwargs):
        x, c, text = self.get_input(batch, self.first_stage_key)
        loss = self(x, c, text)
        return loss

    def forward(self, x, c, text, *args, **kwargs):
        ...
        if self.model.conditioning_key is not None:
            ...
            if self.cond_stage_trainable:
                c = self.get_learned_conditioning(text, c)
                c = self.proj_out(c)
    ...
    @torch.no_grad()
    def log_images(self, batch, N=6, n_row=6, sample=True, ddim_steps=200, ddim_eta=1., return_keys=None,
        ...
        z, c, text, x, xrec, xc, mask, reference = self.get_input(batch, self.first_stage_key,
        ...
        if self.model.conditioning_key is not None:
            if hasattr(self.cond_stage_model, "decode"):
                ...
                log["conditioning"] = xc
                xc = log_txt_as_img((x.shape[2], x.shape[3]), text)
                log["caption"] = xc
            if ismap(xc):
...
class InversionLatentEfficientDiffusion(DDPM):
    """main class"""
