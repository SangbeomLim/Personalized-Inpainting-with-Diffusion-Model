...
class DDPM(pl.LightningModule):
    ...
    @torch.no_grad()
    def log_images(self, batch, N=6, n_row=6, sample=True, return_keys=None, **kwargs):
        log = dict()
        x = self.get_input(batch, self.first_stage_key)
        N = min(x.shape[0], N)
...
class LatentDiffusion(DDPM):
    ...
    def p_losses(self, x_start, cond, t, noise=None, ):
        ...
        # Note
        # x_samples = self.decode_first_stage(x_noisy[:,:4,:,:])
        # image = (x_samples / 2 + 0.5).clamp(0, 1)
        ...
        # pil_images[0].save("/home/user/Paint-by-Example/models/temp.png")
        #
        # x_samples = self.decode_first_stage(noise)
        # image = (x_samples / 2 + 0.5).clamp(0, 1)
        # # we always cast to float32 as this does not cause significant overhead and is compatible with bfloa16
        # image = image.cpu().permute(0, 2, 3, 1).float().numpy()
        #
        ...
        # else:
        #     pil_images = [Image.fromarray(image) for image in images]
        # pil_images[0].save("/home/user/Paint-by-Example/models/temp_1.png")
    ...
    def configure_optimizers(self):
        ...
        if self.cond_stage_trainable:
            ...
            params = params + list(self.cond_stage_model.parameters()) + list(self.proj_out.parameters())
        if self.learn_logvar:
...
class LatentDiffusionFinetune(DDPM):
    """main class"""
