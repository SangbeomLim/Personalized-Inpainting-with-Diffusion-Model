...
from ldm.data.text_inversion import LsunPretrainTuneImageTextInversion
...
# model_path = "/home/user/Paint-by-Example/experiments/E-CLIP/E-CLIP_MLP_added/checkpoints/last.ckpt"
# config_file = "/home/user/Paint-by-Example/configs/E-CLIP.yaml"
model_path = "/home/user/Paint-by-Example/experiments/caption_pretrain/2022-12-26T06-25-48_pretrain_text_inversion/checkpoints/epoch=000009.ckpt"
config_file = "/home/user/Paint-by-Example/configs/pretrain_text_inversion.yaml"

evaluation_model_name = "Caption_pretrain"

class_label_exist = False
caption_exist=True
...
if class_label_exist:
    ...
    testset = LsunPretrainTuneImageTextInversion(state="test", dataset_dir="/home/user/Paint-by-Example/dataset/lsun",
...
metric_model = AutoModel.from_pretrained("google/vit-base-patch32-224-in21k")
    ...
    with precision_scope("cuda"):
        with model.ema_scope():
            for i in tqdm(range(opt.eval_num)):
                if class_label_exist:
                    img_p, ref_p, mask_p, class_label, bbox = testset.test_get_item(i).values()
                elif caption_exist:
                    img_p, ref_p, mask_p, text, bbox = testset.test_get_item(i).values()
                ...
                if class_label_exist:
                    ...
                    c = model.get_learned_conditioning(text, ref_tensor.to(torch.float16))
                ...
                for i, x_sample in enumerate(x_checked_image_torch):
                    # print(bbox)
                    extended_bbox = resize_img_label(bbox, img_p_origin_size, target_size=(512, 512))
                    # print(extended_bbox,img_p_origin_size)
                    generated_img = x_sample[:, extended_bbox[1]:extended_bbox[3], extended_bbox[0]:extended_bbox[2]]
                    grid = 255. * rearrange(generated_img, 'c h w -> h w c').cpu().numpy()
                    generated_img = Image.fromarray(grid.astype(np.uint8))

                    inputs = extractor(images=generated_img, return_tensors="pt")
                    generated_outputs = metric_model(**inputs)

                    reference_img = ref_p
                    inputs = extractor(images=reference_img, return_tensors="pt")
                    reference_outputs = metric_model(**inputs)

                    cosine = cosine_similarity(generated_outputs.pooler_output, reference_outputs.pooler_output)
                    total_cosine_metric += cosine
                    # # Save grid Image
                    # generated_img.save("/home/user/Paint-by-Example/metric/results/1.png")
                    # all_img = []
                    ...
                    # img.save("/home/user/Paint-by-Example/metric/results/2.png")
                    # input()

with open(f"/home/user/Paint-by-Example/metric/results/{evaluation_model_name}.txt", "w", encoding="utf-8") as f:
    total_cosine_metric = total_cosine_metric / opt.eval_num
    f.write(str(total_cosine_metric))
