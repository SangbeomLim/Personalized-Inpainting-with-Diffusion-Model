...
class_dict = {v: k for k, v in enumerate(class_dict)}


def get_class_label(class_string: str):
    ...
    with torch.no_grad():
        with precision_scope("cuda"):
            with model.ema_scope():
                filename = opt.image_path
                img_p = Image.open("examples/image/" + opt.image_path).convert("RGB")
                image_tensor = get_tensor()(img_p)
                image_tensor = image_tensor.unsqueeze(0)
                ref_p = Image.open("examples/reference/" + opt.reference_path).convert("RGB").resize(
                    (224, 224))  # Resize to 224 *224
                ref_tensor = get_tensor_clip()(ref_p)
                ref_tensor = ref_tensor.unsqueeze(0)
                mask = Image.open("examples/mask/" + opt.mask_path).convert("L")
                ...
                if opt.class_label is not None:
                    class_label = torch.tensor([get_class_label(opt.class_label)]).int().to(device)
                ...
                if not opt.skip_save:
                    for i, x_sample in enumerate(x_checked_image_torch):
                        ...
                        img.save(os.path.join(grid_path, 'grid-' + filename[:-4] + '_' + opt.reference_path + '_' + str(
                            random.randint(0, 1000)) + '.png'))
                        ...
                        inpaint_img.save(
                            os.path.join(sample_path, filename[:-4] + '_' + str(opt.seed) + "_inpaint.png"))
